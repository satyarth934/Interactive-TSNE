...
class InteractivePlot:
    ...
    def create_plot(self):
        ...
        cc = self.colormap()
        self.ptsne.scatter(
            self.tsne_obj[:, 0], self.tsne_obj[:, 1], c=cc[self.tcl - 1], s=5
        )
        self.ptsne.set_title(self.title())
        if self.highlight is not None:
            self.ptsne.scatter(
                self.tsne_obj[self.highlight, 0],
                self.tsne_obj[self.highlight, 1],
                **self.highlightpars,
            )
    ...
    def findclosest(self, x, y):
        return np.argmin(
            (self.tsne_obj[:, 0] - x) ** 2 + (self.tsne_obj[:, 1] - y) ** 2
        )
